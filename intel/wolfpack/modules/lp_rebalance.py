...
REBALANCE_COOLDOWN_TICKS = 12  # minimum ticks between rebalances (1 hour)
...
class LPRebalanceEngine:
    ...
    def advance_tick(self) -> None:
        """Advance the tick counter. Call once per cycle, not per position."""
    ...
    def evaluate(self, position, recommended_range, pool_address: str = "") -> dict | None:
        ...
        Args:
            ...
            pool_address: stable key for cooldown state (survives position ID changes on rebalance)
        ...
        3. Range width changed >50% from recommended (vol shift)

        Respects cooldown between rebalances.
        """
        state_key = pool_address or position.position_id
